...
MySQLTool::~MySQLTool{
    if (m_mysql != NULL){
        MYSQL* tmp = m mysql;m _mysql = NULL;
        mysql_close(tmp);
    }
}
...
bool MysQLTool:: connect(const string& host,const string& user,const string& passwd,const string& db){
    ...
    m mysql = mysql_init(m _mysql);//mysql 默认是3306
    ...
    if (m _mysql != NULL){
        ...
        m_passwd = passwd;
        m_dbname = db;
        mysql_query (m mysql,"set names utf8;");
...
bool MySQLTool::Execute(const string& sql，uint32_t& nAffectedCount,int& nErrno){
    if (m mysql ==NULL){
         if (connect(m _host,m_user,m passwd,m_dbname)== false){
             return QueryResultPtr();
            }
    }
    int ret = mysql_query(m mysql,sql.c_str());
    if (ret){
        uint32_t nErrno = mysql_errno (m _mysql);
